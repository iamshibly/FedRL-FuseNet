# Auto-exported from: 6-arcf-net-baseline-backbone-compare.ipynb
...
# ============================================================
# KAGGLE FULL SCRIPT
# TRUE FL + RL-UCB + RACE-FELCM + CRAF + VGG-16
...
# - Adds ECE, MCE, Brier score, calibration tables
...
print("=" * 118)
print("TRUE FL + RL-UCB + RACE-FELCM + CRAF + VGG-16")
...
MODEL_PATH = os.path.join(OUTDIR, "ARCFNet_VGG16_KAGGLE_FULLINFO_checkpoint.pth")
CSV_PATH   = os.path.join(OUTDIR, "ALL_OUTPUTS_AND_METRICS_VGG16_KAGGLE.csv")
...
METHOD_INFO = {
    ...
    "backbone_full_form": "Visual Geometry Group-16",
}
...
# ============================================================
# STEP 8: MODEL — VGG-16 + CRAF Fusion
# ============================================================
print("\n" + "=" * 118)
print("STEP 8: MODEL — VGG-16 + CRAF Fusion")
print("=" * 118)
...
class VGG16CRAF(nn.Module):
    def __init__(self, num_classes, cond_dim=64, fuse_dim=256, embed_dim=256, pretrained=True):
        ...
        self.backbone_blocks = nn.ModuleList([
            nn.Sequential(*feats[:5]),     # block1
            nn.Sequential(*feats[5:10]),   # block2
            nn.Sequential(*feats[10:17]),  # block3
            nn.Sequential(*feats[17:24]),  # block4
            nn.Sequential(*feats[24:31]),  # block5
        ])
...
def set_trainable_for_round(model, rnd):
    for p in model.backbone_blocks.parameters():
        p.requires_grad = False

    for n, p in model.named_parameters():
        if not n.startswith("backbone_blocks."):
            p.requires_grad = True

    if rnd > CFG["freeze_backbone_rounds"]:
        tail_blocks = list(model.backbone_blocks)[-CFG["unfreeze_last_blocks"]:]
        for blk in tail_blocks:
            for p in blk.parameters():
                p.requires_grad = True

def freeze_backbone_bn_stats(model):
    for m in model.backbone_blocks.modules():
...
def make_optimizer(model):
    ...
    for n, p in model.named_parameters():
        if not p.requires_grad:
            continue
        if n.startswith("backbone_blocks."):
            bb_params.append(p)
        else:
...
print("Backbone: VGG-16 | pretrained_loaded=True")
...
print("Transfer backbone: VGG-16")
...
# ============================================================
# KAGGLE FULL SCRIPT
# TRUE FL + RL-UCB + RACE-FELCM + CRAF + DenseNet-121
# METHOD ACRONYM: ARCF-Net
# FULL FORM: Adaptive RACE-FELCM with CRAF Fusion Network
# ------------------------------------------------------------
# KAGGLE-READY + SYMMETRIC DS1/DS2 IMPORTANCE
...
# - Adds MCC, Kappa, PPV, NPV, specificity, FPR, FNR, balanced acc, Jaccard
# - Adds ECE, MCE, Brier score, calibration tables
# - Adds error-analysis tables for BOTH datasets
# - Saves checkpoint WITH full-process info for later XAI / replay
# - PLOTS DISABLED BY DEFAULT to avoid runtime plot crashes
# ============================================================
...
print("=" * 118)
print("TRUE FL + RL-UCB + RACE-FELCM + CRAF + DenseNet-121")
print("METHOD: ARCF-Net (Adaptive RACE-FELCM with CRAF Fusion Network)")
print("=" * 118)
print(f"ENV: {'KAGGLE' if IS_KAGGLE else 'NON-KAGGLE'} | DEVICE: {DEVICE} | torch={torch.__version__}")
print("=" * 118)
...
CFG = {
    ...
    "preproc_val_sample_n": 400,
    "before_after_n": 12,
    "make_plots": False,  # disabled as requested
    "calibration_bins": 12,
}

OUTDIR = "/kaggle/working/outputs" if IS_KAGGLE else "/content/outputs"
os.makedirs(OUTDIR, exist_ok=True)
MODEL_PATH = os.path.join(OUTDIR, "ARCFNet_DENSENET121_KAGGLE_FULLINFO_checkpoint.pth")
CSV_PATH   = os.path.join(OUTDIR, "ALL_OUTPUTS_AND_METRICS_DENSENET121_KAGGLE.csv")
...
METHOD_INFO = {
    ...
    "preprocessing_full_form": "Robust Adaptive Context-Enhanced Fuzzy Edge Local Contrast Mapping",
    "fusion_full_form": "Cross-Residual Adaptive Fusion",
    "backbone_full_form": "Dense Convolutional Network-121",
}
...
if DS1_ROOT is None:
    raise RuntimeError(
        ...
        "'orvile/pmram-bangladeshi-brain-cancer-mri-dataset' as Kaggle input."
    )
if DS2_ROOT is None:
    raise RuntimeError(
        "Could not locate DS2 under /kaggle/input. Add "
        "'yassinebazgour/preprocessed-brain-mri-scans-for-tumors-detection' as Kaggle input."
    )
...
# ============================================================
# STEP 8: MODEL — DenseNet-121 + CRAF Fusion
# ============================================================
print("\n" + "=" * 118)
print("STEP 8: MODEL — DenseNet-121 + CRAF Fusion")
print("=" * 118)
...
class DenseNet121CRAF(nn.Module):
    def __init__(self, num_classes, cond_dim=64, fuse_dim=256, embed_dim=256, pretrained=True):
        super().__init__()
        try:
        ...
        self.backbone_blocks = nn.ModuleList([
            nn.Sequential(feats.conv0, feats.norm0, feats.relu0, feats.pool0),
            nn.Sequential(feats.denseblock1, feats.transition1),
            nn.Sequential(feats.denseblock2, feats.transition2),
            nn.Sequential(feats.denseblock3, feats.transition3),
            nn.Sequential(feats.denseblock4, feats.norm5),
        ])
        self.backbone_relu = nn.ReLU(inplace=True)

        self.backbone_dim = net.classifier.in_features
        self.pool = nn.AdaptiveAvgPool2d((1, 1))
...
print("Backbone: DenseNet-121 | pretrained_loaded=True")
print(f"Total params: {total_params:,}")
print(f"Trainable params: {trainable_params:,} ({(100.0 * trainable_params / total_params):.2f}%)")
...
print(f"Rounds: {CFG['rounds']} | Local epochs: {CFG['local_epochs']}")
print(f"Augmentation ON: {CFG['use_augmentation']}")
print("Transfer backbone: DenseNet-121")
print("Preprocessing: RACE-FELCM")
print("Fusion: CRAF")
print(f"FedProx μ={CFG['fedprox_mu']} | Proto λ={CFG['proto_lambda']}")
print(f"Tempered FedAvg exponent = {CFG['fedavg_temper']:.2f}")
...
# ============================================================
# STEP 14: BEFORE vs AFTER PREPROCESSING (DISABLED)
# ============================================================
print("\n" + "=" * 118)
print("STEP 14: BEFORE vs AFTER PREPROCESSING IMAGES — SKIPPED (plots disabled)")
print("=" * 118)

# ============================================================
# STEP 15: FINAL REPORT PLOTS (DISABLED) + SAFE CM NORMALIZATION EXAMPLE
# ============================================================
print("\n" + "=" * 118)
print("STEP 15: FINAL REPORT PLOTS — SKIPPED (plots disabled)")
print("=" * 118)

# Safe normalization example for future use:
...
# ============================================================
# STEP 16: EVOLUTION PLOTS (DISABLED) BUT TABLES KEPT
# ============================================================
print("\n" + "=" * 118)
print("STEP 16: EVOLUTION TABLES")
print("=" * 118)
...
print_table(theta_evo_df, "Mean selected preprocessing parameters over rounds")
add_table_to_csv(theta_evo_df, "theta_evolution_mean")
...
checkpoint = {
    ...
    "global_round_metrics_table": glob_df.to_dict(orient="list"),
    "local_round_metrics_table": loc_df.to_dict(orient="list"),
    "theta_evolution_table": theta_evo_df.to_dict(orient="list"),
    "all_tables_long_format": all_df.to_dict(orient="list"),
...
# ============================================================
# KAGGLE FULL SCRIPT
# TRUE FL + RL-UCB + RACE-FELCM + CRAF + ResNeXt-50 (32x4d)
# METHOD ACRONYM: ARCF-Net
# FULL FORM: Adaptive RACE-FELCM with CRAF Fusion Network
# ------------------------------------------------------------
# KAGGLE-READY + SYMMETRIC DS1/DS2 IMPORTANCE
...
print("=" * 118)
print("TRUE FL + RL-UCB + RACE-FELCM + CRAF + ResNeXt-50 (32x4d)")
print("METHOD: ARCF-Net (Adaptive RACE-FELCM with CRAF Fusion Network)")
print("=" * 118)
print(f"ENV: {'KAGGLE' if IS_KAGGLE else 'NON-KAGGLE'} | DEVICE: {DEVICE} | torch={torch.__version__}")
print("=" * 118)
...
OUTDIR = "/kaggle/working/outputs" if IS_KAGGLE else "/content/outputs"
os.makedirs(OUTDIR, exist_ok=True)
MODEL_PATH = os.path.join(OUTDIR, "ARCFNet_RESNEXT50_32X4D_KAGGLE_FULLINFO_checkpoint.pth")
CSV_PATH   = os.path.join(OUTDIR, "ALL_OUTPUTS_AND_METRICS_RESNEXT50_32X4D_KAGGLE.csv")
...
METHOD_INFO = {
    ...
    "preprocessing_full_form": "Robust Adaptive Context-Enhanced Fuzzy Edge Local Contrast Mapping",
    "fusion_full_form": "Cross-Residual Adaptive Fusion",
    "backbone_full_form": "ResNeXt-50 (32x4d)",
}
...
# ============================================================
# STEP 8: MODEL — ResNeXt-50 (32x4d) + CRAF Fusion
# ============================================================
print("\n" + "=" * 118)
print("STEP 8: MODEL — ResNeXt-50 (32x4d) + CRAF Fusion")
print("=" * 118)
...
print("Backbone: ResNeXt-50 (32x4d) | pretrained_loaded=True")
print(f"Total params: {total_params:,}")
print(f"Trainable params: {trainable_params:,} ({(100.0 * trainable_params / total_params):.2f}%)")
...
print(f"Rounds: {CFG['rounds']} | Local epochs: {CFG['local_epochs']}")
print(f"Augmentation ON: {CFG['use_augmentation']}")
print("Transfer backbone: ResNeXt-50 (32x4d)")
print("Preprocessing: RACE-FELCM")
print("Fusion: CRAF")
print(f"FedProx μ={CFG['fedprox_mu']} | Proto λ={CFG['proto_lambda']}")
print(f"Tempered FedAvg exponent = {CFG['fedavg_temper']:.2f}")
...
# ============================================================
# KAGGLE FULL SCRIPT
# TRUE FL + RL-UCB + RACE-FELCM + CRAF + EfficientNet-B0
# METHOD ACRONYM: ARCF-Net
# FULL FORM: Adaptive RACE-FELCM with CRAF Fusion Network
# ------------------------------------------------------------
# KAGGLE-READY + SYMMETRIC DS1/DS2 IMPORTANCE
...
print("=" * 118)
print("TRUE FL + RL-UCB + RACE-FELCM + CRAF + EfficientNet-B0")
print("METHOD: ARCF-Net (Adaptive RACE-FELCM with CRAF Fusion Network)")
print("=" * 118)
print(f"ENV: {'KAGGLE' if IS_KAGGLE else 'NON-KAGGLE'} | DEVICE: {DEVICE} | torch={torch.__version__}")
print("=" * 118)
...
OUTDIR = "/kaggle/working/outputs" if IS_KAGGLE else "/content/outputs"
os.makedirs(OUTDIR, exist_ok=True)
MODEL_PATH = os.path.join(OUTDIR, "ARCFNet_EFFICIENTNETB0_KAGGLE_FULLINFO_checkpoint.pth")
CSV_PATH   = os.path.join(OUTDIR, "ALL_OUTPUTS_AND_METRICS_EFFICIENTNETB0_KAGGLE.csv")
...
METHOD_INFO = {
    ...
    "preprocessing_full_form": "Robust Adaptive Context-Enhanced Fuzzy Edge Local Contrast Mapping",
    "fusion_full_form": "Cross-Residual Adaptive Fusion",
    "backbone_full_form": "EfficientNet-B0",
}
...
# ============================================================
# STEP 8: MODEL — EfficientNet-B0 + CRAF Fusion
# ============================================================
print("\n" + "=" * 118)
print("STEP 8: MODEL — EfficientNet-B0 + CRAF Fusion")
print("=" * 118)
...
class EfficientNetB0CRAF(nn.Module):
    def __init__(self, num_classes, cond_dim=64, fuse_dim=256, embed_dim=256, pretrained=True):
        super().__init__()
        try:
        ...
        self.backbone_dim = net.classifier[1].in_features
        self.pool = nn.AdaptiveAvgPool2d(1)
...
global_model = EfficientNetB0CRAF(
    num_classes=NUM_CLASSES,
    cond_dim=64,
    fuse_dim=256,
    embed_dim=256,
...
print("Backbone: EfficientNet-B0 | pretrained_loaded=True")
print(f"Total params: {total_params:,}")
print(f"Trainable params: {trainable_params:,} ({(100.0 * trainable_params / total_params):.2f}%)")
...
print(f"Rounds: {CFG['rounds']} | Local epochs: {CFG['local_epochs']}")
print(f"Augmentation ON: {CFG['use_augmentation']}")
print("Transfer backbone: EfficientNet-B0")
print("Preprocessing: RACE-FELCM")
print("Fusion: CRAF")
print(f"FedProx μ={CFG['fedprox_mu']} | Proto λ={CFG['proto_lambda']}")
print(f"Tempered FedAvg exponent = {CFG['fedavg_temper']:.2f}")
...
for rnd in range(1, CFG["rounds"] + 1):
    ...
    for cid in selected_ids:
        ...
        local_model = EfficientNetB0CRAF(
            num_classes=NUM_CLASSES,
            cond_dim=64,
            fuse_dim=256,
            embed_dim=256,
...
# ============================================================
# KAGGLE FULL SCRIPT
# TRUE FL + RL-UCB + RACE-FELCM + CRAF + MobileNetV3-Large
# METHOD ACRONYM: ARCF-Net
# FULL FORM: Adaptive RACE-FELCM with CRAF Fusion Network
# ------------------------------------------------------------
# KAGGLE-READY + SYMMETRIC DS1/DS2 IMPORTANCE
...
print("=" * 118)
print("TRUE FL + RL-UCB + RACE-FELCM + CRAF + MobileNetV3-Large")
print("METHOD: ARCF-Net (Adaptive RACE-FELCM with CRAF Fusion Network)")
print("=" * 118)
print(f"ENV: {'KAGGLE' if IS_KAGGLE else 'NON-KAGGLE'} | DEVICE: {DEVICE} | torch={torch.__version__}")
print("=" * 118)
...
OUTDIR = "/kaggle/working/outputs" if IS_KAGGLE else "/content/outputs"
os.makedirs(OUTDIR, exist_ok=True)
MODEL_PATH = os.path.join(OUTDIR, "ARCFNet_MOBILENETV3LARGE_KAGGLE_FULLINFO_checkpoint.pth")
CSV_PATH   = os.path.join(OUTDIR, "ALL_OUTPUTS_AND_METRICS_MOBILENETV3LARGE_KAGGLE.csv")
...
METHOD_INFO = {
    ...
    "preprocessing_full_form": "Robust Adaptive Context-Enhanced Fuzzy Edge Local Contrast Mapping",
    "fusion_full_form": "Cross-Residual Adaptive Fusion",
    "backbone_full_form": "MobileNetV3-Large",
}
...
# ============================================================
# STEP 8: MODEL — MobileNetV3-Large + CRAF Fusion
# ============================================================
print("\n" + "=" * 118)
print("STEP 8: MODEL — MobileNetV3-Large + CRAF Fusion")
print("=" * 118)
...
class MobileNetV3LargeCRAF(nn.Module):
    def __init__(self, num_classes, cond_dim=64, fuse_dim=256, embed_dim=256, pretrained=True):
        super().__init__()
        try:
        ...
        self.backbone = net.features
        self.backbone_dim = net.classifier[0].in_features
        self.pool = nn.AdaptiveAvgPool2d(1)
...
global_model = MobileNetV3LargeCRAF(
    num_classes=NUM_CLASSES,
    cond_dim=64,
    fuse_dim=256,
    embed_dim=256,
...
print("Backbone: MobileNetV3-Large | pretrained_loaded=True")
print(f"Total params: {total_params:,}")
print(f"Trainable params: {trainable_params:,} ({(100.0 * trainable_params / total_params):.2f}%)")
...
print(f"Rounds: {CFG['rounds']} | Local epochs: {CFG['local_epochs']}")
print(f"Augmentation ON: {CFG['use_augmentation']}")
print("Transfer backbone: MobileNetV3-Large")
print("Preprocessing: RACE-FELCM")
print("Fusion: CRAF")
print(f"FedProx μ={CFG['fedprox_mu']} | Proto λ={CFG['proto_lambda']}")
print(f"Tempered FedAvg exponent = {CFG['fedavg_temper']:.2f}")
...
for rnd in range(1, CFG["rounds"] + 1):
    ...
    for cid in selected_ids:
        ...
        local_model = MobileNetV3LargeCRAF(
            num_classes=NUM_CLASSES,
            cond_dim=64,
            fuse_dim=256,
            embed_dim=256,
...
# ============================================================
# KAGGLE FULL SCRIPT
# TRUE FL + RL-UCB + RACE-FELCM + CRAF + ConvNeXt-Tiny
# METHOD ACRONYM: ARCF-Net
# FULL FORM: Adaptive RACE-FELCM with CRAF Fusion Network
# ------------------------------------------------------------
# KAGGLE-READY + SYMMETRIC DS1/DS2 IMPORTANCE
...
print("=" * 118)
print("TRUE FL + RL-UCB + RACE-FELCM + CRAF + ConvNeXt-Tiny")
print("METHOD: ARCF-Net (Adaptive RACE-FELCM with CRAF Fusion Network)")
print("=" * 118)
print(f"ENV: {'KAGGLE' if IS_KAGGLE else 'NON-KAGGLE'} | DEVICE: {DEVICE} | torch={torch.__version__}")
print("=" * 118)
...
OUTDIR = "/kaggle/working/outputs" if IS_KAGGLE else "/content/outputs"
os.makedirs(OUTDIR, exist_ok=True)
MODEL_PATH = os.path.join(OUTDIR, "ARCFNet_CONVNEXTTINY_KAGGLE_FULLINFO_checkpoint.pth")
CSV_PATH   = os.path.join(OUTDIR, "ALL_OUTPUTS_AND_METRICS_CONVNEXTTINY_KAGGLE.csv")
...
METHOD_INFO = {
    ...
    "preprocessing_full_form": "Robust Adaptive Context-Enhanced Fuzzy Edge Local Contrast Mapping",
    "fusion_full_form": "Cross-Residual Adaptive Fusion",
    "backbone_full_form": "ConvNeXt-Tiny",
}
...
# ============================================================
# STEP 8: MODEL — ConvNeXt-Tiny + CRAF Fusion
# ============================================================
print("\n" + "=" * 118)
print("STEP 8: MODEL — ConvNeXt-Tiny + CRAF Fusion")
print("=" * 118)
...
class ConvNeXtTinyCRAF(nn.Module):
    def __init__(self, num_classes, cond_dim=64, fuse_dim=256, embed_dim=256, pretrained=True):
        super().__init__()
        try:
        ...
        self.backbone = net.features
        self.backbone_dim = net.classifier[2].in_features
        self.pool = nn.AdaptiveAvgPool2d(1)
...
print("Backbone: ConvNeXt-Tiny | pretrained_loaded=True")
print(f"Total params: {total_params:,}")
print(f"Trainable params: {trainable_params:,} ({(100.0 * trainable_params / total_params):.2f}%)")
...
print(f"Rounds: {CFG['rounds']} | Local epochs: {CFG['local_epochs']}")
print(f"Augmentation ON: {CFG['use_augmentation']}")
print("Transfer backbone: ConvNeXt-Tiny")
print("Preprocessing: RACE-FELCM")
print("Fusion: CRAF")
print(f"FedProx μ={CFG['fedprox_mu']} | Proto λ={CFG['proto_lambda']}")
print(f"Tempered FedAvg exponent = {CFG['fedavg_temper']:.2f}")
...
# ============================================================
# KAGGLE FULL SCRIPT
# TRUE FL + RL-UCB + RACE-FELCM + CRAF + Swin-Tiny
# METHOD ACRONYM: ARCF-Net
# FULL FORM: Adaptive RACE-FELCM with CRAF Fusion Network
# ------------------------------------------------------------
# KAGGLE-READY + SYMMETRIC DS1/DS2 IMPORTANCE
...
print("=" * 118)
print("TRUE FL + RL-UCB + RACE-FELCM + CRAF + Swin-Tiny")
print("METHOD: ARCF-Net (Adaptive RACE-FELCM with CRAF Fusion Network)")
print("=" * 118)
print(f"ENV: {'KAGGLE' if IS_KAGGLE else 'NON-KAGGLE'} | DEVICE: {DEVICE} | torch={torch.__version__}")
print("=" * 118)
...
OUTDIR = "/kaggle/working/outputs" if IS_KAGGLE else "/content/outputs"
os.makedirs(OUTDIR, exist_ok=True)
MODEL_PATH = os.path.join(OUTDIR, "ARCFNet_SWINTINY_KAGGLE_FULLINFO_checkpoint.pth")
CSV_PATH   = os.path.join(OUTDIR, "ALL_OUTPUTS_AND_METRICS_SWINTINY_KAGGLE.csv")
...
# ============================================================
# STEP 8: MODEL — Swin-Tiny + CRAF Fusion
# ============================================================
print("\n" + "=" * 118)
print("STEP 8: MODEL — Swin-Tiny + CRAF Fusion")
print("=" * 118)
...
class SwinTinyCRAF(nn.Module):
    def __init__(self, num_classes, cond_dim=64, fuse_dim=256, embed_dim=256, pretrained=True):
        ...
        self.backbone = net.features
        self.backbone_norm = net.norm
        self.backbone_post = nn.Sequential(
            PermuteLayer(0, 3, 1, 2),
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(1),
        )
        self.backbone_dim = net.head.in_features
...
def set_trainable_for_round(model, rnd):
    ...
    for n, p in model.named_parameters():
        if not (n.startswith("backbone.") or n.startswith("backbone_norm.")):
            p.requires_grad = True
...
def make_optimizer(model):
    ...
    for n, p in model.named_parameters():
        if not p.requires_grad:
            continue
        if n.startswith("backbone.") or n.startswith("backbone_norm."):
            bb_params.append(p)
        else:
            head_params.append(p)
...
print("Backbone: Swin-Tiny | pretrained_loaded=True")
print(f"Total params: {total_params:,}")
print(f"Trainable params: {trainable_params:,} ({(100.0 * trainable_params / total_params):.2f}%)")
...
print(f"Rounds: {CFG['rounds']} | Local epochs: {CFG['local_epochs']}")
print(f"Augmentation ON: {CFG['use_augmentation']}")
print("Transfer backbone: Swin-Tiny")
print("Preprocessing: RACE-FELCM")
print("Fusion: CRAF")
print(f"FedProx μ={CFG['fedprox_mu']} | Proto λ={CFG['proto_lambda']}")
print(f"Tempered FedAvg exponent = {CFG['fedavg_temper']:.2f}")
